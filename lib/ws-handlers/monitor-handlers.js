...
export default function registerMonitorHandlers({ socket, handlers, consciousnessEngine, ensureEngineInitialized }) {
 ...
 if (socket.emit && socket.emit.mock) {
  ...
  // USER ACTION: Start monitoring session handler
  const startMonitoringHandler = async (data) => {
    try {
  ...
  // Register validated start monitoring handler
  socket.on('start-monitoring', wrapHandler(startMonitoringHandler, 'start-monitoring'));

  // Monitor get characters handler
  const monitorGetCharactersHandler = async () => {
    try {
  ...
  // Register validated get characters handler
  socket.on('monitor:get-characters', wrapHandler(monitorGetCharactersHandler, 'monitor:get-characters'));

  // Monitor start handler
  const monitorStartHandler = async (data) => {
    try {
      const { characterId } = data;
      if (!characterId) {
        socket.emit('error', { message: 'Character ID required' });
  ...
  // Register validated monitor start handler
  socket.on('monitor:start', wrapHandler(monitorStartHandler, 'monitor:start'));

  // Stop monitoring handler
  const stopMonitoringHandler = async (data) => {
  ...
  // Register validated stop monitoring handler
  socket.on('stop-monitoring', wrapHandler(stopMonitoringHandler, 'stop-monitoring'));

  // Refresh monitor handler
  const refreshMonitorHandler = async () => {
    try {
  ...
  // Register validated refresh monitor handler
  socket.on('refresh-monitor', wrapHandler(refreshMonitorHandler, 'refresh-monitor'));

  // Get system resources handler
  const getSystemResourcesHandler = async () => {
  ...
  // Register validated get system resources handler
  socket.on('get-system-resources', wrapHandler(getSystemResourcesHandler, 'get-system-resources'));

  // Get error logs handler
  const getErrorLogsHandler = async () => {
  ...
  // Register validated get error logs handler
  socket.on('get-error-logs', wrapHandler(getErrorLogsHandler, 'get-error-logs'));

  // Get memory allocation handler
  const getMemoryAllocationHandler = async () => {
  ...
  // Register validated get memory allocation handler
  socket.on('get-memory-allocation', wrapHandler(getMemoryAllocationHandler, 'get-memory-allocation'));
