...
import { wsDebugCommandsLimiter } from '../middleware/rate-limiter.js';

export default function registerDebugHandlers({ socket, io, ensureEngineInitialized, consciousnessEngine }) {
  // Debug command handler with validation and rate limiting
    ...
    try {
      ...
      if (!rateLimitResult.allowed) {
        socket.emit('debug-result', {
          ...
          success: false,
          result: {
            error: 'Rate limit exceeded. Too many debug commands.',
  ...
  // Undo last command handler with rate limiting
  ...
  // Redo last undone command handler with rate limiting
  ...
  // Get command history handler with rate limiting
  ...
  // Get undo/redo status handler with rate limiting
