  ...
  return {
    consciousness: {
      ...
      resources: {
        ...
        // FIXED: Use consistent memory reporting
        memory: {
          // Process memory usage (what processes are consuming)
          processUsage: systemResources.totalMemoryUsage || 0,
          processPercentage: systemResources.memoryPercentage || 0,
          
          // Total system memory (memory manager pools)
          used: memoryStatus.capacity?.used || this.usage.memory,
          total: memoryStatus.capacity?.total || this.resources.memory.total,
          available: memoryStatus.capacity?.available || (this.resources.memory.total - this.usage.memory),
          percentage: memoryStatus.capacity ? 
            (memoryStatus.capacity.used / memoryStatus.capacity.total) * 100 : 
            (this.usage.memory / this.resources.memory.total) * 100
        },
        threads: {
