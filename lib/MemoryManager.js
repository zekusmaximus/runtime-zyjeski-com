...
class MemoryManager {
    constructor(consciousnessInstance, memoryMapConfig = {}, dependencies = {}) {
    ...
    allocateMemory(memoryData, type = 'shortTerm') {
        ...
        // Emit MemoryAllocated event
        if (this.eventBus) {
            this.eventBus.emit('MemoryAllocated', {
                blockId: memoryId,
                address: memoryBlock.address || memoryId,
                size: memorySize,
                type: type,
                processId: memoryData.processId || null,
    ...
    calculateMemorySize(memoryData) {
        return MemoryCalculator.calculateMemorySize(memoryData);
    }
    ...
    getIntensityRange(intensity) {
        return MemoryCalculator.getIntensityRange(intensity);
    }
    ...
    calculateCorruptionRisk(memory) {
        return MemoryCalculator.calculateCorruptionRisk(memory);
    }
    ...
    triggerMemoryPressure() {
        ...
        if (MemoryCalculator.shouldTriggerPressureCleanup(this.memoryCapacity.available, this.memoryCapacity.reserved)) {
    ...
    compressOldMemories() {
        ...
        this.memorySegments.forEach(memory => {
            if (!memory.compressed && 
                ...
                memory.size = MemoryCalculator.calculateCompressedSize(memory.size);
                this.updateCapacityMetrics();
            }
        });
    }
    ...
    calculateMemoryValue(memory) {
        return MemoryCalculator.calculateMemoryValue(memory);
    }
    ...
    // Alias for defragmentMemory to match memory-state.js interface
    defragment(aggressive = false) {
        return this.defragmentMemory();
    }
    ...
    async tick() {
        ...
        const availablePercentage = MemoryCalculator.calculateAvailablePercentage(this.memoryCapacity.available, this.memoryCapacity.total);
    ...
    getMemoryDataForFrontend() {
        const memoryData = {
            capacity: {
                ...
                utilizationPercentage: MemoryCalculator.calculateUtilizationPercentage(this.memoryCapacity.allocated, this.memoryCapacity.total)
            },
            pools: {},
            fragmentation: {
                level: this.fragmentationLevel,
                severity: MemoryCalculator.getFragmentationSeverity(this.fragmentationLevel)
            },
            compression: {
        ...
        Object.entries(this.memoryPools).forEach(([poolType, pool]) => {
            memoryData.pools[poolType] = {
                ...
                utilizationPercentage: MemoryCalculator.calculatePoolUtilization(pool.size, pool.maxSize || 2000),
...
class MemoryLeakDetector {
    ...
    isLeakCandidate(memory, pattern, currentTime) {
        return MemoryCalculator.isLeakCandidate(memory, currentTime);
    }
    
    calculateLeakScore(memory, pattern) {
        return MemoryCalculator.calculateLeakScore(memory, Date.now());
    }
}
