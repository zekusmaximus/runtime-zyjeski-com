...
export class CommandExecutor extends EventEmitter {
     ...
     * @param {Object} dependencies.eventBus - EventBus instance (optional)
     */
    ...
    async execute(command, context = {}) {
        ...
        try {
            ...
            // Emit execution event (legacy EventEmitter)
            ...
            // Emit CommandExecuted event through EventBus
            if (this.eventBus) {
                this.eventBus.emit('CommandExecuted', {
                    commandId: command.id || `cmd_${Date.now()}`,
            ...
            // Emit error event (legacy EventEmitter)
            ...
            // Emit CommandExecuted event through EventBus (for failures)
            if (this.eventBus) {
                this.eventBus.emit('CommandExecuted', {
                    commandId: command.id || `cmd_${Date.now()}`,
                    commandType: command.constructor.name,
