...
class ConditionEvaluator {
    constructor(consciousnessInstance) {
        ...
        // Initialize expression parser for safe mathematical evaluation
    ...
    // Safely evaluate mathematical expression using expr-eval
    safeEvaluate(expression) {
        try {
            // Handle logical operators by converting to mathematical equivalents
            let processed = expression;

            // Convert logical operators to mathematical operations
            processed = processed.replace(/&&/g, ' * ');  // AND becomes multiplication
            processed = processed.replace(/\|\|/g, ' + '); // OR becomes addition (with max 1)
            ...
            if (expression.includes('||')) {
                ...
                if (!this.mathParser.functions.min) {
                    this.mathParser.functions.min = (...args) => Math.min(...args);
                }
            }

            const expr = this.mathParser.parse(processed);
            const result = expr.evaluate({});
            return Boolean(result);
        } catch (error) {
            console.warn('Math expression evaluation error:', error.message, 'Expression:', expression);
            return false;
        }
    }
