...
class DatabaseManager {
  ...
  updateDebuggingState(stateId, updateData) {
    ...
    if (updateData.consciousnessState !== undefined) {
      fields.push('consciousness_state = ?');
      values.push(JSON.stringify(updateData.consciousnessState));
    }
    if (updateData.terminalHistory !== undefined) {
      fields.push('terminal_history = ?');
      values.push(JSON.stringify(updateData.terminalHistory));
    }
    if (updateData.breakpoints !== undefined) {
      fields.push('breakpoints = ?');
      values.push(JSON.stringify(updateData.breakpoints));
    }
