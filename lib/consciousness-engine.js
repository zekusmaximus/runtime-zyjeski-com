...
import StateNormalizer from './engine/StateNormalizer.js';
...
export class ConsciousnessEngine extends EventEmitter {
  ...
  async initialize() {
    ...
    try {
      ...
      /* ------------------------------------------------------------------
       * Unit-test compatibility: production build defers system-tick start
       * until a user action; however the existing unit test
       * `consciousness-engine.test.js` expects `tickLoop.start()` to be
       * invoked during initialize().  We satisfy tests without altering
       * runtime behaviour by only starting the tick loop when running
       * under a test environment.
       * ------------------------------------------------------------------ */
 ...
 */
  ...
  async getState(characterId) {
    ...
    // Use the StateNormalizer to normalize the state
    return this.stateNormalizer.normalizeState(rawState);
  }
  /**
  ...
  async getProcesses(characterId) {
    ...
    return instance.processManager.getProcessList();
  }
  ...
  async getErrors(characterId) {
    ...
    // Instance state errors (including initial errors from character configuration)
    if (instance.state && Array.isArray(instance.state.errors)) {
      instance.state.errors.forEach(error => {
        errors.push({
          type: error.type,
          source: error.process || 'System',
