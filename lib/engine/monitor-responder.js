export class MonitorResponder {
  ...
  async broadcastStateChange(characterId, changeType = 'state-change') {
    ...
    for (const [socketId, monitoring] of this.monitoringSockets) {
      ...
      /* ----------------------------------------------------------
       * Emit fine-grained monitor events expected by the new client
       * ---------------------------------------------------------- */
      const base   = state?.consciousness || state || {};
      const res    = base.resources || {};
      const memory = base.memory    || {};
      const errors = base.system_errors || base.errors || [];

      this.engine.emit('resources-update', {
        socketId,
        characterId,
        cpu: res.cpu,
        threads: res.threads,
      ...
      this.engine.emit('memory-update', {
        socketId,
        characterId,
        allocationByProcess: memory.pools || memory.allocationByProcess || {}
      });

      this.engine.emit('errors-update', {
        socketId,
        characterId,
        list: errors
      });

      /* Existing consolidated update for backwards compatibility */
  ...
  async startMonitoring(characterId, socketId) {
    ...
    /* Immediately seed client dashboards with the same granular events */
    const baseInit   = initialState.consciousness || initialState || {};
    const resInit    = baseInit.resources || {};
    const memoryInit = baseInit.memory    || {};
    const errorsInit = baseInit.system_errors || [];

    this.engine.emit('resources-update', {
      socketId,
      characterId,
      cpu: resInit.cpu,
      threads: resInit.threads,
      memTotal: resInit.memory
    });

    this.engine.emit('memory-update', {
      socketId,
      characterId,
      allocationByProcess: memoryInit.pools || memoryInit.allocationByProcess || {}
    });
