...
export class CSPConfig {
  ...
  getScriptSources(nonce) {
    ...
    // Always include nonce support for secure scripts
    if (nonce) {
      sources.push(`'nonce-${nonce}'`);
    }

    // In development, allow additional flexibility
    if (this.environment === 'development') {
      // Allow unsafe-inline for dynamically created scripts in development
  ...
  getStyleSources(nonce) {
    ...
    // Always include nonce support for secure styles
    if (nonce) {
      sources.push(`'nonce-${nonce}'`);
    }
    ...
    // In development, allow additional flexibility
    if (this.environment === 'development') {
      // Allow unsafe-inline for dynamically created styles in development
      sources.push("'unsafe-inline'");
      sources.push('https://cdnjs.cloudflare.com');
    }
  ...
  validateConfig() {
    ...
    Object.entries(testDirectives).forEach(([directive, sources]) => {
      if (Array.isArray(sources)) {
        // Only flag unsafe-inline as an issue in production
        if (sources.includes("'unsafe-inline'") && this.environment === 'production') {
          issues.push(`${directive} contains 'unsafe-inline' - major security risk in production`);
        }
        // unsafe-eval should never be allowed (we eliminated all usage)
        ...
        // Add development warnings instead of errors
        if (sources.includes("'unsafe-inline'") && this.environment === 'development') {
          warnings.push(`${directive} contains 'unsafe-inline' - acceptable in development but should be avoided in production`);
        }
      }
    });
