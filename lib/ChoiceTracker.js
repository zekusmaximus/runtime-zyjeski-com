...
export class ChoiceTracker extends EventEmitter {
    constructor() {
        ...
        this.setupExpressionFunctions();
    }

    /**
     * Setup custom functions for expression evaluation
     */
    setupExpressionFunctions() {
        // These functions will be available in choice conditions
        this.expressionParser.functions.getDebugIssueCount = () => {
            // This will be set dynamically in evaluateSimpleCondition
            return 0;
        };

        this.expressionParser.functions.playerActionCount = (actionType) => {
            // This will be set dynamically in evaluateSimpleCondition
            return 0;
        };
    }
    ...
    /**
     * Evaluate simple condition using safe expression parser
     */
    evaluateSimpleCondition(condition, context) {
        try {
            // Handle function calls by replacing with actual values
            let processed = condition;

            // Replace getDebugIssueCount() with actual value
            const debugIssueCount = context.consciousness.debugIssueCount || 0;
            processed = processed.replace(/getDebugIssueCount\(\)/g, debugIssueCount.toString());

            // Replace playerActionCount() with actual value
            const actionCount = context.playerChoices?.size || 0;
            processed = processed.replace(/playerActionCount\([^)]*\)/g, actionCount.toString());

            // Handle logical operators by converting to mathematical equivalents
            // First convert comparison results to 0/1 values
            processed = processed.replace(/(\d+)\s*([><=!]+)\s*(\d+)/g, (match, left, op, right) => {
                ...
                switch (op) {
                    ...
                    case '==': result = l === r ? 1 : 0; break;
                    case '!=': result = l !== r ? 1 : 0; break;
                    default: result = match; break;
                }
                return result.toString();
            });

            processed = processed.replace(/&&/g, ' * ');  // AND becomes multiplication
            processed = processed.replace(/\|\|/g, ' + '); // OR becomes addition
            processed = processed.replace(/!/g, '(1 - '); // NOT becomes (1 - x)

            // Count NOT operators to add closing parentheses
            const notCount = (condition.match(/!/g) || []).length;
            processed += ')'.repeat(notCount);

            // Handle OR operations by clamping to 1
            if (condition.includes('||')) {
                processed = `min(1, ${processed})`;
                // Add min function if not exists
                if (!this.expressionParser.functions.min) {
                    this.expressionParser.functions.min = (...args) => Math.min(...args);
                }
            }

            // Parse and evaluate the condition safely
            const expr = this.expressionParser.parse(processed);
            return Boolean(expr.evaluate({}));
        } catch (error) {
            console.warn('Choice condition evaluation error:', error.message, 'Condition:', condition);
            return false;
        }
    }
