...
import WebSocketEventBridge from './events/WebSocketEventBridge.js';
...
const consciousnessEngine = new ConsciousnessEngine({ eventBus });
...
class WebSocketHandlers {
  ...
  initializeHandlers(socket, io) {
    ...
    // Initialize WebSocketEventBridge if not already created
    if (!this.eventBridge) {
      this.eventBridge = new WebSocketEventBridge(eventBus, io, {
        enableDebugLogging: process.env.NODE_ENV !== 'production',
        filterSensitiveData: true,
        maxEventRate: 100
      });
      info('WebSocketEventBridge initialized');
    }
    ...
    // Handle client disconnection for event bridge
    socket.on('disconnect', () => {
      this.connectedSockets.delete(socket.id);
      if (this.eventBridge) {
        this.eventBridge.handleClientDisconnect(socket.id);
      }
    });
